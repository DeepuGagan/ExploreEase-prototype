import React from 'react'

const LatestStoriesParis = () => {
  return (
    <section className="jsx-f704f42be0b5c3bb max-w-full mx-4 my-16 articles lg:my-24 md:mx-auto md:container cursor-default-hover">
      <div className="jsx-f704f42be0b5c3bb cursor-default-hover">
        <header className="jsx-f704f42be0b5c3bb flex items-end justify-between mt-5 mb-10 lg:mt-8 lg:mb-14">
          <h2 className="text-2xl md:text-5xl md:leading-relaxed">
            Latest stories{' '}
            <span className="inline-block">
              from Paris
            </span>
          </h2>
          <div className="jsx-f704f42be0b5c3bb flex-none hidden md:inline-block">
            <a
              className="jsx-f704f42be0b5c3bb btn"
              href="/france/paris/articles"
            >
              Read more articles
            </a>
          </div>
        </header>
      </div>
      <div className="jsx-f704f42be0b5c3bb relative z-0 lg:flex lg:gap-x-8">
        <div className="jsx-f704f42be0b5c3bb mb-8 lg:w-3/5 lg:shrink-0 lg:mb-auto">
          <div className="relative">
            <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
              <a
                className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                href="/api/auth/login?action=bookmark"
              >
                <span className="sr-only">
                  Login
                </span>
                <svg
                  aria-hidden="true"
                  className="text-white"
                  fill="currentColor"
                  height="1em"
                  viewBox="0 0 14 20"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                </svg>
              </a>
              <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                <div className="jsx-d9256db7ff0ace36">
                  <h4 className="text-lg font-semibold leading-tight">
                    Save your favorites!
                  </h4>
                  <p className="text-sm">
                    <a href="https://www.lonelyplanet.com/api/auth/login">
                      Sign in
                    </a>
                    {' '}to save, get inso & more
                  </p>
                </div>
              </div>
            </div>
            <div className="relative lg:rounded overflow-hidden lg:h-[688px]">
              <img
                alt="The Saint-Honore pastry from Philippe Conticini, Paris"
                className="max-w-full hidden object-cover w-full h-full rounded -z-10 lg:block"
                data-nimg="1"
                decoding="async"
                height="630"
                loading="lazy"
                src="https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=730&h=630&fit=crop&q=75"
                srcSet="https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=730&h=630&fit=crop&q=75 1x, https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=730&h=630&fit=crop&q=75 2x"
                style={{
                  color: 'transparent'
                }}
                width="730"
              />
              <img
                alt="The Saint-Honore pastry from Philippe Conticini, Paris"
                className="max-w-full object-cover -z-10 rounded lg:hidden w-full aspect-[350/200]"
                data-nimg="1"
                decoding="async"
                height="425"
                loading="lazy"
                src="https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=780&h=425&fit=crop&q=75"
                srcSet="https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=780&h=425&fit=crop&q=75 1x, https://lp-cms-production.imgix.net/2024-05/Paris-best-pastries.png?auto=format&w=780&h=425&fit=crop&q=75 2x"
                style={{
                  color: 'transparent'
                }}
                width="780"
              />
              <div className="top-0 flex items-end w-full h-full pt-4 lg:absolute lg:bg-gradient-to-t from-transparent-80 via-transparent-40 to-transparent lg:p-8">
                <div className="lg:w-2/3 lg:pb-8">
                  <p className="relative z-10 uppercase label-sm text-black-400 lg:text-white">
                    Food
                  </p>
                  <a
                    className="inline-block my-2 font-semibold card-link heading-04 lg:text-white cursor-on-hover"
                    href="/articles/where-to-find-best-pastries-paris"
                  >
                    The 6 best pastries in Paris - and where to find them
                  </a>
                  <p className="relative text-black-400 lg:text-white">
                    May 24, 2024 • 5 min read
                  </p>
                  <p className="relative hidden mt-4 lg:block lg:text-white">
                    The best pastries in Paris right now.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="jsx-3406129628 max-w-full overflow-x-hidden px-4 lg:container w-full carousel-wrapper lg:px-0">
          <ul className="jsx-3406129628 carousel flex overflow-x-scroll md:overflow-x-auto overflow-y-hidden -mx-4">
            <li className="jsx-3406129628 carousel-item flex-shrink-0 px-4 pb-2 w-full md:w-full">
              <div className="jsx-f704f42be0b5c3bb flex flex-col gap-y-4 lg:h-full">
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white cursor-on-hover"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="PARIS, FRANCE - OCTOBER 28: A dancer performs during the 'Totally Crazy !' show at Le Crazy Horse on October 28, 2021 in Paris, France. (Photo by Marc Piasecki/Getty Images for ABA)1350219551bestof, topix"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2023-05/GettyImages-1350219551.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2023-05/GettyImages-1350219551.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Art and Culture
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/escape-the-obvious-paris-cabaret-crazy-horse-moulin-rouge"
                      >
                        Escape the obvious in Paris: pick the Crazy Horse cabaret over the (temporarily windmill-less) Moulin Rouge
                      </a>
                      <p className="text-sm text-black-400">
                        Apr 25, 2024 • 5 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt=""
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-04/Paris-Local-Strolls-Featured.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-04/Paris-Local-Strolls-Featured.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Walking Tour
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/local-strolls-paris-walking-tour"
                      >
                        Local Strolls: Follow our writer's perfect walking route in Paris
                      </a>
                      <p className="text-sm text-black-400">
                        Apr 12, 2024 • 4 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="521911032.French, landscape architecture, three people, Caucasian ethnicity, footbridge, ornamental garden, male, pond, Giverny, Claude Monet521911032Getty,  RFE,  French,  footbridge,  male,  pond,  Giverny,  Caucasian ethnicity,  Claude Monet,  landscape architecture,  ornamental garden,  three people,  Flower,  Garden,  Land,  Nature,  Outdoors,  Person,  Pond,  Tree,  Vegetation,  Water"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-03/GettyImages-521911032-cropped.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-03/GettyImages-521911032-cropped.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Day Trip
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/the-best-day-trips-from-paris"
                      >
                        7 easy day trips from Paris we love
                      </a>
                      <p className="text-sm text-black-400">
                        Apr 1, 2024 • 8 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Young Asian woman embracing her little girl on the street. Bonding between mother and daughter. Family lifestyle. Love and care concept.1815725593"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-03/GettyImages-1815725593.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-03/GettyImages-1815725593.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Art
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/paris-on-a-budget"
                      >
                        Paris on a budget: affordable ways to experience the city
                      </a>
                      <p className="text-sm text-black-400">
                        Mar 31, 2024 • 10 min read
                      </p>
                    </div>
                  </article>
                </div>
              </div>
            </li>
            <li className="jsx-3406129628 carousel-item flex-shrink-0 px-4 pb-2 w-full md:w-full">
              <div className="jsx-f704f42be0b5c3bb flex flex-col gap-y-4 lg:h-full">
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Tourists in Paris planning their trip using map465929843Adult, Beautiful, Caucasian, Chair, Cheerful, City, Confusion, Eiffel Tower, Europe, Famous Place, Formal Garden, France, Friendship, Get Lost, Group Of People, Joy, Lifestyles, Map, Outdoors, Paris - France, Park, People, Planning, Street, Three People, Tourism, Tourist, Tuileries Quarter, Urban Scene, Women"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-03/GettyImages-465929843.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-03/GettyImages-465929843.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Destination Practicalities
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/first-time-paris"
                      >
                        13 things you should know before traveling to Paris
                      </a>
                      <p className="text-sm text-black-400">
                        Mar 31, 2024 • 7 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Young black women walking in Paris in the city center talking and texting on the phone and commuting using one of public city bikes.579744192Beautiful, African Descent, Capital Cities, Real People, Candid, One Woman Only, Young Women, Women, Females, Searching, City Life, Youth Culture, Downtown District, Bookstore, Kiosk, 20-29 Years, Young Adult, Adult, Smiling, Looking, Market Stall, One Person, Cool, Carefree, Happiness, Modern, Lifestyles, Outdoors, Cheerful, People, Paris - France, France, Europe, Summer, Springtime, Workshop, Store, City Street, Street, Book"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-03/GettyImages-579744192.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-03/GettyImages-579744192.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Festivals & Events
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/best-time-to-visit-paris"
                      >
                        When is the best time to go to Paris?
                      </a>
                      <p className="text-sm text-black-400">
                        Mar 29, 2024 • 6 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Shot of a young woman touring the city of Paris with a bicycle1180138804"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2023-10/GettyImages-1180138804.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2023-10/GettyImages-1180138804.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Destination Practicalities
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/getting-around-paris"
                      >
                        How to navigate your way around Paris
                      </a>
                      <p className="text-sm text-black-400">
                        Mar 27, 2024 • 6 min read
                      </p>
                    </div>
                  </article>
                </div>
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Cremieux Street (Rue Cremieux), Paris, France. Rue Cremieux in the 12th Arrondissement is one of the prettiest residential streets in Paris. Colored houses in Rue Cremieux street in Paris. France.; Shutterstock ID 1487451410; GL: 65050; netsuite: Online Editorial; full: Paris neighbourhoods; name: Emma / Sparks1487451410Cremieux Street (Rue Cremieux), Paris, France. Rue Cremieux in the 12th Arrondissement is one of the prettiest residential streets in Paris. Colored houses in Rue Cremieux street in Paris. France."
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2024-03/shutterstock1487451410.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2024-03/shutterstock1487451410.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Destination Practicalities
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/articles/guide-paris-arrondissements"
                      >
                        Our guide to the 20 arrondissements of Paris
                      </a>
                      <p className="text-sm text-black-400">
                        Mar 27, 2024 • 15 min read
                      </p>
                    </div>
                  </article>
                </div>
              </div>
            </li>
            <li className="jsx-3406129628 carousel-item flex-shrink-0 px-4 pb-2 w-full md:w-full">
              <div className="jsx-f704f42be0b5c3bb flex flex-col gap-y-4 lg:h-full">
                <div className="relative">
                  <div className="absolute top-0 right-0 z-30 m-3 rounded-full group">
                    <a
                      className="flex items-center justify-center w-10 h-10 p-2 rounded-full text-md bg-black/50 group-hover:bg-black"
                      href="/api/auth/login?action=bookmark"
                    >
                      <span className="sr-only">
                        Login
                      </span>
                      <svg
                        aria-hidden="true"
                        className="text-white"
                        fill="currentColor"
                        height="1em"
                        viewBox="0 0 14 20"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M12 0.625H2C1.50272 0.625 1.02581 0.822544 0.674175 1.17417C0.322544 1.52581 0.125 2.00272 0.125 2.5V18.75C0.125941 18.8607 0.15514 18.9693 0.20983 19.0656C0.264519 19.1618 0.342886 19.2425 0.4375 19.3C0.535537 19.3525 0.645032 19.38 0.75625 19.38C0.867468 19.38 0.976963 19.3525 1.075 19.3L7 15.725L12.925 19.2875C13.0228 19.3471 13.1355 19.3774 13.25 19.375C13.3585 19.3741 13.4654 19.3485 13.5625 19.3C13.6571 19.2425 13.7355 19.1618 13.7902 19.0656C13.8449 18.9693 13.8741 18.8607 13.875 18.75V2.5C13.875 2.00272 13.6775 1.52581 13.3258 1.17417C12.9742 0.822544 12.4973 0.625 12 0.625ZM12.625 17.65L7.325 14.4625C7.22619 14.4054 7.1141 14.3754 7 14.3754C6.8859 14.3754 6.77381 14.4054 6.675 14.4625L1.375 17.65V2.5C1.375 2.33424 1.44085 2.17527 1.55806 2.05806C1.67527 1.94085 1.83424 1.875 2 1.875H12C12.1658 1.875 12.3247 1.94085 12.4419 2.05806C12.5592 2.17527 12.625 2.33424 12.625 2.5V17.65Z" />
                      </svg>
                    </a>
                    <div className="jsx-d9256db7ff0ace36 c-tooltip invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-md absolute w-contents min-w-64 right-0 translate-x-2 mt-3 px-4 pt-2 pb-3 shadow">
                      <div className="jsx-d9256db7ff0ace36">
                        <h4 className="text-lg font-semibold leading-tight">
                          Save your favorites!
                        </h4>
                        <p className="text-sm">
                          <a href="https://www.lonelyplanet.com/api/auth/login">
                            Sign in
                          </a>
                          {' '}to save, get inso & more
                        </p>
                      </div>
                    </div>
                  </div>
                  <article className="relative flex items-center rounded-md h-full p-4 hover:shadow-md focus:shadow-md md:my-6 bg-[#F0F7FC] border border-[#E0EEFA] md:my-auto lg:h-[160px]">
                    <div className="flex-none mr-4 md:mr-6 rounded-md overflow-hidden w-1/5">
                      <img
                        alt="Paris, france , may 28 2022 ; typical vintage book sellers from the Seine riverbanks in Paris , bookworm and touristic lifestyle; Shutterstock ID 2162521935; your: Brian Healy; gl: 65050; netsuite: Lonely Planet Online Editorial; full: Paris bouquinistes2162521935"
                        className="max-w-full object-cover w-full h-full aspect-square"
                        data-nimg="1"
                        decoding="async"
                        height="140"
                        loading="lazy"
                        src="https://lp-cms-production.imgix.net/2023-08/shutterstock2162521935.jpg?auto=format&w=140&h=140&fit=crop&q=75"
                        srcSet="https://lp-cms-production.imgix.net/2023-08/shutterstock2162521935.jpg?auto=format&w=140&h=140&fit=crop&q=75 1x"
                        style={{
                          color: 'transparent'
                        }}
                        width="140"
                      />
                    </div>
                    <div className="flex-auto w-0">
                      <div className="text-sm uppercase font-semibold tracking-wide relative z-10 mb-2 w-90 text-black-400 block">
                        Books
                      </div>
                      <a
                        className="lg:text-lg text-black hover:text-black font-semibold card-link mb-1 line-clamp-3"
                        href="/news/paris-bouquinistes-booksellers-seine-threatened"
                      >
                        Paris's beloved booksellers secure victory to stay put during this summer's Olympics
                      </a>
                      <p className="text-sm text-black-400">
                        Feb 15, 2024 • 4 min read
                      </p>
                    </div>
                  </article>
                </div>
              </div>
            </li>
          </ul>
          <div className="jsx-3406129628 md:flex items-center mt-4 hidden md:justify-end">
            <div className="flex text-black-300">
              <button
                aria-label="previous slide"
                className="cursor-default opacity-50 mr-2"
                disabled
                type="button"
              >
                <svg
                  aria-hidden="true"
                  className="text-3xl"
                  fill="currentColor"
                  height="1em"
                  viewBox="0 0 40 40"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M20 1.25C16.2916 1.25 12.6665 2.34967 9.58307 4.40994C6.49964 6.47022 4.09641 9.39857 2.67727 12.8247C1.25812 16.2508 0.886812 20.0208 1.61028 23.6579C2.33376 27.2951 4.11952 30.636 6.74176 33.2583C9.36399 35.8805 12.7049 37.6663 16.3421 38.3897C19.9792 39.1132 23.7492 38.7419 27.1753 37.3227C30.6014 35.9036 33.5298 33.5004 35.5901 30.4169C37.6503 27.3335 38.75 23.7084 38.75 20C38.75 15.0272 36.7746 10.2581 33.2583 6.74175C29.742 3.22544 24.9728 1.25 20 1.25ZM20 36.25C16.7861 36.25 13.6443 35.297 10.972 33.5114C8.29969 31.7258 6.21689 29.1879 4.98697 26.2186C3.75704 23.2493 3.43524 19.982 4.06225 16.8298C4.68926 13.6776 6.23692 10.7821 8.50952 8.50951C10.7821 6.23691 13.6776 4.68925 16.8298 4.06224C19.982 3.43523 23.2493 3.75703 26.2186 4.98696C29.1879 6.21688 31.7258 8.29969 33.5114 10.972C35.297 13.6443 36.25 16.7861 36.25 20C36.2434 24.3077 34.5292 28.4371 31.4832 31.4832C28.4372 34.5292 24.3077 36.2434 20 36.25Z" />
                  <path d="M23.3994 12.7501C23.2832 12.6329 23.145 12.5399 22.9926 12.4764C22.8403 12.413 22.6769 12.3803 22.5119 12.3803C22.3469 12.3803 22.1835 12.413 22.0312 12.4764C21.8789 12.5399 21.7406 12.6329 21.6244 12.7501L14.8494 19.1001C14.6113 19.339 14.4775 19.6627 14.4775 20.0001C14.4775 20.3374 14.6113 20.6611 14.8494 20.9001L21.6244 27.3001C21.7353 27.4125 21.8677 27.5015 22.0138 27.5616C22.1598 27.6218 22.3165 27.6518 22.4744 27.6501C22.6481 27.6528 22.8205 27.6185 22.98 27.5496C23.1394 27.4806 23.2824 27.3785 23.3994 27.2501C23.5158 27.136 23.6083 26.9999 23.6714 26.8496C23.7346 26.6994 23.7671 26.538 23.7671 26.3751C23.7671 26.2121 23.7346 26.0508 23.6714 25.9005C23.6083 25.7503 23.5158 25.6141 23.3994 25.5001L17.4994 20.0001L23.3494 14.5001C23.4691 14.3894 23.5656 14.2559 23.633 14.1074C23.7005 13.959 23.7377 13.7986 23.7423 13.6356C23.747 13.4726 23.719 13.3103 23.6602 13.1582C23.6013 13.0062 23.5126 12.8674 23.3994 12.7501Z" />
                </svg>
              </button>
              <button
                aria-label="next slide"
                className=""
                type="button"
              >
                <svg
                  aria-hidden="true"
                  className="text-3xl"
                  fill="currentColor"
                  height="1em"
                  viewBox="0 0 40 40"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M20 38.75C23.7084 38.75 27.3335 37.6503 30.4169 35.5901C33.5004 33.5298 35.9036 30.6014 37.3227 27.1753C38.7419 23.7492 39.1132 19.9792 38.3897 16.3421C37.6663 12.7049 35.8805 9.36399 33.2583 6.74176C30.636 4.11952 27.2951 2.33376 23.6579 1.61028C20.0208 0.886812 16.2508 1.25812 12.8247 2.67727C9.39857 4.09641 6.47022 6.49964 4.40994 9.58307C2.34967 12.6665 1.25 16.2916 1.25 20C1.25 24.9728 3.22544 29.742 6.74175 33.2583C10.2581 36.7746 15.0272 38.75 20 38.75ZM20 3.75001C23.2139 3.75001 26.3557 4.70305 29.028 6.48863C31.7003 8.2742 33.7831 10.8121 35.013 13.7814C36.243 16.7507 36.5648 20.018 35.9378 23.1702C35.3107 26.3224 33.7631 29.2179 31.4905 31.4905C29.2179 33.7631 26.3224 35.3108 23.1702 35.9378C20.018 36.5648 16.7507 36.243 13.7814 35.013C10.8121 33.7831 8.27419 31.7003 6.48862 29.028C4.70305 26.3557 3.75 23.214 3.75 20C3.75661 15.6923 5.47078 11.5629 8.51682 8.51683C11.5629 5.47079 15.6923 3.75662 20 3.75001Z" />
                  <path d="M16.6006 27.25C16.7176 27.3785 16.8606 27.4806 17.0201 27.5496C17.1795 27.6185 17.3519 27.6528 17.5256 27.65C17.6835 27.6518 17.8402 27.6217 17.9862 27.5616C18.1323 27.5015 18.2647 27.4125 18.3756 27.3L25.1506 20.9C25.3887 20.6611 25.5225 20.3374 25.5225 20C25.5225 19.6627 25.3887 19.339 25.1506 19.1L18.3756 12.7C18.2594 12.5829 18.1211 12.4899 17.9688 12.4264C17.8165 12.363 17.6531 12.3303 17.4881 12.3303C17.3231 12.3303 17.1597 12.363 17.0074 12.4264C16.855 12.4899 16.7168 12.5829 16.6006 12.7C16.4842 12.8141 16.3917 12.9502 16.3286 13.1005C16.2654 13.2507 16.2329 13.4121 16.2329 13.575C16.2329 13.738 16.2654 13.8993 16.3286 14.0496C16.3917 14.1998 16.4842 14.336 16.6006 14.45L22.5006 20L16.6506 25.5C16.5309 25.6107 16.4344 25.7442 16.367 25.8927C16.2995 26.0411 16.2624 26.2015 16.2577 26.3645C16.253 26.5275 16.281 26.6898 16.3399 26.8419C16.3987 26.9939 16.4874 27.1327 16.6006 27.25Z" />
                </svg>
              </button>
            </div>
          </div>
        </div>
        <div className="jsx-f704f42be0b5c3bb flex justify-center space-x-4">
          <a
            className="jsx-f704f42be0b5c3bb block w-full mt-6 text-center lg:hidden btn"
            href="/france/paris/articles"
          >
            Read more articles
          </a>
        </div>
      </div>
    </section>
  )
}

export default LatestStoriesParis